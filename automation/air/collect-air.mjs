...
function fetchJsonInsecure(url) {
  return new Promise((resolve, reject) => {
    const req = https.get(url, {
      headers: { 'user-agent': 'phuket-navi-air-collector/1.0 (+https://phuket.sunsun.live/)' },
...
// プーケット県内の観測局はAir4Thai上で1局のみ(タラートヤイ/プーケットタウン中心部)。
const STATIONS = [
  { id: '43t', label: 'Phuket Town (Talat Yai)' },
];
...
async function main() {
  ...
  if (stations.length === 0) throw new Error('プーケットの観測局データを1件も取得できなかったため、既存データを保持します。');
